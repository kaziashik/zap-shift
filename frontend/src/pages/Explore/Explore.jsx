...
const Explore = () => {
    ...
    const filteredServices = useMemo(() => {
        ...
        if (priceBand === 'under-80') list = list.filter((s) => s.priceFrom < 80);
        if (priceBand === '80-120') list = list.filter((s) => s.priceFrom >= 80 && s.priceFrom <= 120);
        if (priceBand === 'over-120') list = list.filter((s) => s.priceFrom > 120);
    ...
    const filteredCenters = useMemo(() => {
        ...
        if (statusFilter !== 'all') list = list.filter((c) => String(c.status || 'active') === statusFilter);
        if (sort === 'name') list.sort((a, b) => String(a.district).localeCompare(String(b.district)));
        return list;
    }, [centers, search, region, statusFilter, sort]);
    ...
    }, [tab, search, category, priceBand, region, statusFilter, sort, reset]);
    ...
    return (
        <div className="pb-10">
            ...
            <div className="zs-surface mb-6 grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-5">
                ...
                {tab === 'services' ? (
                    <>
                        ...
                        <label className="form-control">
                            ...
                            <select className="select select-bordered rounded-xl" value={priceBand} onChange={(e) => setPriceBand(e.target.value)}>
