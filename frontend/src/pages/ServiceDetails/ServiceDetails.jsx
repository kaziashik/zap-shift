    ...
    const reviews = useMemo(() => {
        const fromApi = apiReviews
            ...
            .map((r) => ({
                ...
                rating: r.rating,
                review: r.comment || 'Great delivery experience with ZapShift.'
            }));
        if (fromApi.length) return fromApi;
        return (reviewsData || []).slice(0, 3);
    }, [apiReviews]);
        ...
        <div className="space-y-8 pb-12">
            <section className="grid gap-6 lg:grid-cols-2">
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[service.image, related[0]?.image, related[1]?.image].filter(Boolean).slice(0, 3).map((src, i) => (
                            <img key={`${src}-${i}`} src={src} alt="" className="h-20 w-full rounded-xl object-cover" />
                        ))}
                    </div>
                </div>
            ...
            <section className="zs-surface p-6">
                ...
                <p className="mt-3 text-base-content/75">
                    ...
                    Senders get tracking after payment; admins coordinate riders; riders confirm pickup and delivery with OTP.
                </p>
            </section>
            ...
            <section className="zs-surface p-6">
                <h2 className="text-xl font-bold text-secondary dark:text-primary">Reviews & ratings</h2>
                <p className="mt-2 text-sm text-base-content/70">Recent feedback from ZapShift senders.</p>
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                    {reviews.map((item) => (
                        <article key={item.id || item.name} className="rounded-2xl border border-base-300 bg-base-100 p-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                        {related.map((item) => (
                            <Link key={item.id} to={`/services/${item.id}`} className="zs-card hover:border-primary">
                                ...
                                <span className="zs-btn-primary btn-sm mt-auto">View Details</span>
                            </Link>
                        ))}
                    </div>
                </section>
